#!/bin/bash
#
# SubioHub Installation Script
# SubioHub 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/dlxyz/SubioHub/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="dlxyz/SubioHub"
INSTALL_DIR="/opt/subiohub"
SERVICE_NAME="subiohub"
SERVICE_USER="subiohub"
CONFIG_DIR="/etc/subiohub"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="SubioHub 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 SubioHub 服务："
    ...
    ["not_installed"]="SubioHub 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 SubioHub。"
    ...
    ["uninstall_complete"]="SubioHub 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 SubioHub"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 SubioHub"
    ...
    ["server_config_desc"]="配置 SubioHub 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="SubioHub Installation Script"
    ...
    # Completion
    ["install_complete"]="SubioHub installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start SubioHub service:"
    ...
    ["not_installed"]="SubioHub is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove SubioHub from your system."
    ...
    ["uninstall_complete"]="SubioHub has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install SubioHub"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove SubioHub"
    ...
    ["server_config_desc"]="Configure SubioHub server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/subiohub" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/subiohub" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="subiohub_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/subiohub" "$INSTALL_DIR/subiohub"
    chmod +x "$INSTALL_DIR/subiohub"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/subiohub"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/subiohub.service << EOF
[Unit]
Description=SubioHub - AI API Gateway Platform
Documentation=https://github.com/dlxyz/SubioHub
...
User=subiohub
Group=subiohub
WorkingDirectory=/opt/subiohub
ExecStart=/opt/subiohub/subiohub
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=subiohub
...
start_service() {
    ...
    if systemctl start subiohub; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u subiohub -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable subiohub 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status subiohub"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u subiohub -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart subiohub"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop subiohub"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if SubioHub is installed
    if [ ! -f "$INSTALL_DIR/subiohub" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/subiohub" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet subiohub; then
        print_info "$(msg 'stopping_service')"
        systemctl stop subiohub
    fi

    # Backup current binary
    cp "$INSTALL_DIR/subiohub" "$INSTALL_DIR/subiohub.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/subiohub.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/subiohub"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start subiohub

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: SubioHub must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/subiohub" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="subiohub.backup.${current_version}"
        else
            backup_name="subiohub.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/subiohub" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop subiohub 2>/dev/null || true
    systemctl disable subiohub 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/subiohub.service
    systemctl daemon-reload
