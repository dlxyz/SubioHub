...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/subiohub/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/subiohub"
DATA_DIR="/var/lib/subiohub/datamanagement"
SERVICE_FILE_NAME="subiohub-datamanagementd.service"
...
参数:
  ...
  --source  指定 subiohub 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/subiohub-src
EOF
}
...
if ! id subiohub >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 subiohub..."
  useradd --system --no-create-home --shell /usr/sbin/nologin subiohub
else
  echo "[2/6] 系统用户 subiohub 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R subiohub:subiohub /var/lib/subiohub
chmod 0750 "$DATA_DIR"
...
systemctl enable --now subiohub-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status subiohub-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u subiohub-datamanagementd -f
2. 在 subiohub（容器部署时）挂载 socket:
