...
`subiohub` 采用经典的分层架构，后端主要负责高并发的 API 代理、流量控制和计费，`next-web` 负责用户控制台、管理后台与公开站展示。
...
| **并发模型** | **Goroutine (协程)**：多线程，能同时榨干 CPU 所有核心。极其适合做高并发的 API 网关。 | **单线程事件循环**：非阻塞 I/O。处理普通 Web 接口很快，但遇到密集计算（如加密解密、大规模文本分词）会卡住主线程。 | **同 Node.js**，主要专注于全栈 React 渲染和 Web 同构。 |
| **运行环境** | 丢到 Linux 服务器直接跑，不需要任何依赖。 | 服务器必须安装 Node.js 环境。 | 服务器必须安装 Node.js，且占用内存较大（你之前遇到的 OOM 崩溃就是证明）。 |
| **核心定位** | **高性能后台服务器、网关、微服务**。 | 中小型后端 API、中间件、脚本。 | **SEO 友好的全栈网站、Web 页面**。 |

**总结：** Go 语言天生就是为了做像 `subiohub` 这种需要疯狂转发请求、计算 Token、高并发不宕机的**“脏活累活后台”**而生的。而 Next.js 则是为了做**给普通用户看、需要被百度/谷歌搜索到的漂亮网站**而生的。两者结合（Go 做纯后端 API，Next.js 做 Web 展示）是目前最完美的架构。
