...
type createChannelRequest struct {
	...
	ProviderType               string                           `json:"provider_type" binding:"omitempty,oneof=standard deepseek qwen doubao zhipu kimi minimax openai_compat_domestic custom_domestic"`
...
type testDomesticConnectionRequest struct {
...
func channelToResponse(ch *service.Channel) *channelResponse {
	...
	resp := &channelResponse{
		...
		ProviderConfig: ch.ProviderConfig,
...
func (h *ChannelHandler) List(c *gin.Context) {
	...
	providerScope := strings.TrimSpace(c.Query("provider_scope"))
	if len(search) > 100 {
		search = search[:100]
	}
...
// TestDomesticConnection handles testing a domestic channel upstream connection.
// POST /api/v1/admin/channels/test-connection
func (h *ChannelHandler) TestDomesticConnection(c *gin.Context) {
	...
	var req testDomesticConnectionRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.ErrorFrom(c, infraerrors.BadRequest("VALIDATION_ERROR", err.Error()))
		return
	}

	result, err := h.domesticChannelService.TestConnection(c.Request.Context(), req.ProviderType, req.ProviderConfig, req.TestModel)
	if err != nil {
		response.ErrorFrom(c, infraerrors.BadRequest("DOMESTIC_CHANNEL_TEST_FAILED", err.Error()))
		return
	}
