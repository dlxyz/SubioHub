...
type OpenAIGatewayHandler struct {
	...
	domesticChannelService  *service.DomesticChannelExecutionService
...
func resolveDomesticMessagesForwardBody(
	body []byte,
	...
	if mappedModel := strings.TrimSpace(preferredMappedModel); mappedModel != "" {
		return gatewayService.ReplaceModelInBody(body, mappedModel)
	}
	return body
}

func resolveDomesticMessagesUsageFields(
	channelMapping service.ChannelMappingResult,
	requestedModel string,
	upstreamModel string,
	preferredMappedModel string,
) service.ChannelUsageFields {
	fields := channelMapping.ToUsageFields(requestedModel, upstreamModel)
	if channelMapping.Mapped {
		return fields
	}

	mappedModel := strings.TrimSpace(preferredMappedModel)
	...
	fields.ChannelMappedModel = mappedModel
	if upstreamModel != "" && upstreamModel != mappedModel {
		fields.ModelMappingChain = requestedModel + "→" + mappedModel + "→" + upstreamModel
	} else {
		fields.ModelMappingChain = requestedModel + "→" + mappedModel
	}
	return fields
}
...
func NewOpenAIGatewayHandler(
	...
	return &OpenAIGatewayHandler{
		...
		domesticChannelService:  domesticChannelService,
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	if err != nil {
		reqLog.Warn("openai.responses.domestic_channel_lookup_failed", zap.Error(err))
		h.handleStreamingAwareError(c, http.StatusServiceUnavailable, "api_error", "Channel temporarily unavailable", streamStarted)
		return
	}
	if domesticChannel != nil {
		...
		result, err := h.domesticChannelService.ForwardOpenAIResponses(c.Request.Context(), c, domesticChannel, forwardBody, reqModel)
		if err != nil && !errors.Is(err, service.ErrDomesticResponseWritten) {
			reqLog.Warn("openai.responses.domestic_forward_failed",
				zap.Int64("channel_id", domesticChannel.ID),
				zap.String("provider_type", domesticChannel.ProviderType),
				zap.Error(err),
			)
			h.handleStreamingAwareError(c, http.StatusBadGateway, "upstream_error", "Domestic upstream request failed", streamStarted)
			return
		}
		if result != nil {
			usageAccount, usageErr := h.domesticChannelService.EnsureUsageAccountForChannel(c.Request.Context(), domesticChannel)
			if usageErr != nil {
				reqLog.Warn("openai.responses.domestic_usage_account_prepare_failed",
					zap.Int64("channel_id", domesticChannel.ID),
					zap.Error(usageErr),
				)
				...
				h.submitUsageRecordTask(func(ctx context.Context) {
					if err := h.gatewayService.RecordUsage(ctx, &service.OpenAIRecordUsageInput{
						...
						).Error("openai.responses.domestic_record_usage_failed", zap.Error(err))
					}
				})
			}
		}
	...
	for {
		...
		if previousResponseID != "" && selection.Account != nil {
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	domesticChannel, err := h.domesticChannelService.GetDomesticChannelForGroup(c.Request.Context(), apiKey.GroupID)
	if err != nil {
		reqLog.Warn("openai_messages.domestic_channel_lookup_failed", zap.Error(err))
		h.anthropicStreamingAwareError(c, http.StatusServiceUnavailable, "api_error", "Channel temporarily unavailable", streamStarted)
		return
	}
	if domesticChannel != nil {
		forwardBody := resolveDomesticMessagesForwardBody(body, channelMappingMsg, preferredMappedModel, h.gatewayService)
		result, err := h.domesticChannelService.ForwardAnthropicMessages(c.Request.Context(), c, domesticChannel, forwardBody, reqModel)
		if err != nil && !errors.Is(err, service.ErrDomesticResponseWritten) {
			reqLog.Warn("openai_messages.domestic_forward_failed",
				zap.Int64("channel_id", domesticChannel.ID),
				zap.String("provider_type", domesticChannel.ProviderType),
				zap.Error(err),
			)
			h.anthropicStreamingAwareError(c, http.StatusBadGateway, "api_error", "Domestic upstream request failed", streamStarted)
			return
		}
		if result != nil {
			usageAccount, usageErr := h.domesticChannelService.EnsureUsageAccountForChannel(c.Request.Context(), domesticChannel)
			if usageErr != nil {
				reqLog.Warn("openai_messages.domestic_usage_account_prepare_failed",
					zap.Int64("channel_id", domesticChannel.ID),
					zap.Error(usageErr),
				)
			} else if usageAccount != nil {
				...
				h.submitUsageRecordTask(func(ctx context.Context) {
					if err := h.gatewayService.RecordUsage(ctx, &service.OpenAIRecordUsageInput{
						...
						RequestPayloadHash: requestPayloadHash,
						APIKeyService:      h.apiKeyService,
						ChannelUsageFields: resolveDomesticMessagesUsageFields(
							channelMappingMsg,
							reqModel,
						...
						logger.L().With(
							...
							zap.String("provider_type", domesticChannel.ProviderType),
							zap.String("model", reqModel),
						).Error("openai_messages.domestic_record_usage_failed", zap.Error(err))
					}
				})
			}
		}
		reqLog.Debug("openai_messages.domestic_request_completed",
			zap.Int64("channel_id", domesticChannel.ID),
			zap.String("provider_type", domesticChannel.ProviderType),
		)
		return
