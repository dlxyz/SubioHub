...
func (h *OpenAIGatewayHandler) ChatCompletions(c *gin.Context) {
	...
	if err != nil {
		reqLog.Warn("openai_chat_completions.domestic_channel_lookup_failed", zap.Error(err))
	...
	if domesticChannel != nil {
		...
		result, err := h.domesticChannelService.ForwardOpenAIChatCompletions(c.Request.Context(), c, domesticChannel, forwardBody, reqModel)
		if err != nil && !errors.Is(err, service.ErrDomesticResponseWritten) {
			reqLog.Warn("openai_chat_completions.domestic_forward_failed",
		...
		if result != nil {
			...
			if usageErr != nil {
				reqLog.Warn("openai_chat_completions.domestic_usage_account_prepare_failed",
					zap.Int64("channel_id", domesticChannel.ID),
					zap.Error(usageErr),
				)
				...
				h.submitUsageRecordTask(func(ctx context.Context) {
					if err := h.gatewayService.RecordUsage(ctx, &service.OpenAIRecordUsageInput{
						...
						).Error("openai_chat_completions.domestic_record_usage_failed", zap.Error(err))
					}
				})
			}
		}
		reqLog.Debug("openai_chat_completions.domestic_request_completed",
			zap.Int64("channel_id", domesticChannel.ID),
			zap.String("provider_type", domesticChannel.ProviderType),
		)
		return
