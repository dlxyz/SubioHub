...
// Rerank handles OpenAI-compatible rerank requests for domestic channels.
// POST /v1/rerank
func (h *OpenAIGatewayHandler) Rerank(c *gin.Context) {
	...
	if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), apiKey.User, apiKey, apiKey.Group, subscription); err != nil {
		reqLog.Info("openai_rerank.billing_eligibility_check_failed", zap.Error(err))
	...
	if err != nil {
		reqLog.Warn("openai_rerank.domestic_channel_lookup_failed", zap.Error(err))
		h.errorResponse(c, http.StatusServiceUnavailable, "api_error", "Channel temporarily unavailable")
		return
	}
	if domesticChannel == nil {
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "Domestic rerank is not configured for this group")
		return
	}
	...
	result, err := h.domesticChannelService.ForwardOpenAIRerank(c.Request.Context(), c, domesticChannel, forwardBody, reqModel)
	if err != nil && !errors.Is(err, service.ErrDomesticResponseWritten) {
		reqLog.Warn("openai_rerank.domestic_forward_failed",
	...
	if usageErr != nil {
		reqLog.Warn("openai_rerank.domestic_usage_account_prepare_failed",
			zap.Int64("channel_id", domesticChannel.ID),
			zap.Error(usageErr),
		)
		...
		h.submitUsageRecordTask(func(ctx context.Context) {
			if err := h.gatewayService.RecordUsage(ctx, &service.OpenAIRecordUsageInput{
				...
				UpstreamEndpoint:   EndpointRerank,
				UserAgent:          userAgent,
				IPAddress:          clientIP,
				...
				).Error("openai_rerank.domestic_record_usage_failed", zap.Error(err))
			}
		})
	}

	reqLog.Debug("openai_rerank.domestic_request_completed",
		zap.Int64("channel_id", domesticChannel.ID),
		zap.String("provider_type", domesticChannel.ProviderType),
	)
}
