...
func TestResolveDomesticMessagesForwardBody(t *testing.T) {
	t.Parallel()
	...
	t.Run("uses preferred mapped model when channel mapping is absent", func(t *testing.T) {
		result := resolveDomesticMessagesForwardBody(body, service.ChannelMappingResult{}, "deepseek-v4-flash", handler.gatewayService)
		require.Equal(t, "deepseek-v4-flash", gjson.GetBytes(result, "model").String())
	})

	t.Run("channel mapping takes precedence over preferred mapped model", func(t *testing.T) {
		result := resolveDomesticMessagesForwardBody(body, service.ChannelMappingResult{
			Mapped:      true,
			MappedModel: "deepseek-v4-pro",
		}, "deepseek-v4-flash", handler.gatewayService)
		require.Equal(t, "deepseek-v4-pro", gjson.GetBytes(result, "model").String())
	})

	t.Run("leaves body unchanged when no mapping is available", func(t *testing.T) {
		result := resolveDomesticMessagesForwardBody(body, service.ChannelMappingResult{}, "", handler.gatewayService)
		require.Equal(t, "claude-sonnet-4-20250514", gjson.GetBytes(result, "model").String())
	})
}

func TestResolveDomesticMessagesUsageFields(t *testing.T) {
	t.Parallel()

	t.Run("uses preferred mapped model when only group mapping is present", func(t *testing.T) {
		fields := resolveDomesticMessagesUsageFields(
		...
		require.Equal(t, "claude-sonnet-4-20250514", fields.OriginalModel)
		require.Equal(t, "deepseek-v4-flash", fields.ChannelMappedModel)
		require.Equal(t, service.BillingModelSourceChannelMapped, fields.BillingModelSource)
		require.Equal(t, "claude-sonnet-4-20250514→deepseek-v4-flash", fields.ModelMappingChain)
	})

	t.Run("preserves channel mapping when channel mapping already exists", func(t *testing.T) {
		fields := resolveDomesticMessagesUsageFields(
			service.ChannelMappingResult{
				ChannelID:          1,
				Mapped:             true,
		...
		require.Equal(t, "deepseek-v4-pro", fields.ChannelMappedModel)
		require.Equal(t, "claude-opus-4-20250514→deepseek-v4-pro", fields.ModelMappingChain)
	})
}
