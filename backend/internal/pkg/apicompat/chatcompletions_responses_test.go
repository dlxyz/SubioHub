...
func TestResponsesToChatCompletionsRequest_StringInput(t *testing.T) {
	...
	assert.Equal(t, "deepseek-chat", chatReq.Model)
	require.Len(t, chatReq.Messages, 2)
	assert.Equal(t, "system", chatReq.Messages[0].Role)
	assert.Equal(t, "user", chatReq.Messages[1].Role)

	var systemText string
	require.NoError(t, json.Unmarshal(chatReq.Messages[0].Content, &systemText))
	assert.Equal(t, "You are helpful.", systemText)

	var userText string
	require.NoError(t, json.Unmarshal(chatReq.Messages[1].Content, &userText))
...
func TestResponsesToChatCompletionsRequest_MixedInputAndTools(t *testing.T) {
	strict := true
	req := &ResponsesRequest{
		Model: "deepseek-reasoner",
		Input: json.RawMessage(`[
			{"role":"user","content":[{"type":"input_text","text":"Look at this"},{"type":"input_image","image_url":"https://example.com/cat.png"}]},
			{"role":"assistant","content":[{"type":"output_text","text":"Let me think"}]},
	...
	require.NoError(t, json.Unmarshal(chatReq.Messages[0].Content, &userParts))
	require.Len(t, userParts, 2)
	assert.Equal(t, "text", userParts[0].Type)
	assert.Equal(t, "Look at this", userParts[0].Text)
	require.NotNil(t, userParts[1].ImageURL)
	assert.Equal(t, "https://example.com/cat.png", userParts[1].ImageURL.URL)

	var assistantText string
	require.NoError(t, json.Unmarshal(chatReq.Messages[1].Content, &assistantText))
	assert.Equal(t, "Let me think", assistantText)

	require.Len(t, chatReq.Messages[2].ToolCalls, 1)
	assert.Equal(t, "call_1", chatReq.Messages[2].ToolCalls[0].ID)
	assert.Equal(t, "lookup", chatReq.Messages[2].ToolCalls[0].Function.Name)
	assert.Equal(t, `{"q":"weather"}`, chatReq.Messages[2].ToolCalls[0].Function.Arguments)

	var toolOutput string
	require.NoError(t, json.Unmarshal(chatReq.Messages[3].Content, &toolOutput))
	assert.Equal(t, "sunny", toolOutput)

	require.Len(t, chatReq.Tools, 1)
	assert.Equal(t, "lookup", chatReq.Tools[0].Function.Name)
	assert.Equal(t, &strict, chatReq.Tools[0].Function.Strict)
}

func TestChatCompletionsToResponsesResponse_ToolCallsAndUsage(t *testing.T) {
	...
	assert.Equal(t, "Answer", out.Output[0].Content[0].Text)
	assert.Equal(t, "function_call", out.Output[1].Type)
	assert.Equal(t, "call_1", out.Output[1].CallID)
	assert.Equal(t, "lookup", out.Output[1].Name)
	assert.Equal(t, `{"q":"weather"}`, out.Output[1].Arguments)
	...
	assert.Equal(t, 5, out.Usage.InputTokensDetails.CachedTokens)
}

func TestChatCompletionsToResponsesResponse_Incomplete(t *testing.T) {
	resp := &ChatCompletionsResponse{
		Model: "deepseek-chat",
		Choices: []ChatChoice{{
...
func TestChatChunkToResponsesEvents_TextToolAndCompletion(t *testing.T) {
	...
	events := ChatChunkToResponsesEvents(chunk, state)
	...
	assert.Equal(t, `{"q":"wea`, events[4].Delta)
	...
	assert.Equal(t, `ther"}`, events[0].Delta)
	assert.Equal(t, "response.output_text.done", events[1].Type)
	assert.Equal(t, "Hello", events[1].Text)
	assert.Equal(t, "response.output_item.done", events[2].Type)
	assert.Equal(t, "message", events[2].Item.Type)
	assert.Equal(t, "response.function_call_arguments.done", events[3].Type)
	assert.Equal(t, `{"q":"weather"}`, events[3].Arguments)
	assert.Equal(t, "response.output_item.done", events[4].Type)
	assert.Equal(t, "function_call", events[4].Item.Type)
	assert.Equal(t, "response.completed", events[5].Type)
	require.NotNil(t, events[5].Response)
	assert.Equal(t, "completed", events[5].Response.Status)
	require.NotNil(t, events[5].Response.Usage)
	assert.Equal(t, 10, events[5].Response.Usage.InputTokens)
	assert.Equal(t, 4, events[5].Response.Usage.OutputTokens)

	assert.Nil(t, FinalizeChatResponsesStream(state))
}
