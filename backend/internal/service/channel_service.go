...
type ChannelRepository interface {
	...
	List(ctx context.Context, params pagination.PaginationParams, status, search, providerScope string) ([]Channel, *pagination.PaginationResult, error)
...
func (s *ChannelService) Create(ctx context.Context, input *CreateChannelInput) (*Channel, error) {
	...
	if channel.ProviderType == "" {
		channel.ProviderType = ProviderTypeStandard
	}
...
func (s *ChannelService) applyUpdateInput(ctx context.Context, channel *Channel, input *UpdateChannelInput) error {
	...
	if input.ProviderType != nil {
		channel.ProviderType = normalizeProviderType(*input.ProviderType)
	}
	if input.ProviderConfig != nil {
		channel.ProviderConfig = input.ProviderConfig
	}
...
// List 获取渠道列表
func (s *ChannelService) List(ctx context.Context, params pagination.PaginationParams, status, search, providerScope string) ([]Channel, *pagination.PaginationResult, error) {
	return s.repo.List(ctx, params, status, search, providerScope)
}
