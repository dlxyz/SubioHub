...
func TestMain(m *testing.M) {
	...
	pgContainer, err := tcpostgres.Run(
		ctx,
		postgresImage,
		tcpostgres.WithDatabase("subiohub_test"),
...
func truncateIntegrationTables(t testing.TB, tables ...string) {
	t.Helper()

	require.NotEmpty(t, tables, "truncate requires at least one table")
...
func uniqueTestEmail(base string) string {
	...
	if at <= 0 || at == len(base)-1 {
		return fmt.Sprintf("%s-%d@example.com", sanitizeRedisNamespace(base), atomic.AddUint64(&testEmailSeq, 1))
	}

	local := sanitizeRedisNamespace(base[:at])
	domain := base[at+1:]
	return fmt.Sprintf("%s-%d@%s", local, atomic.AddUint64(&testEmailSeq, 1), domain)
}
