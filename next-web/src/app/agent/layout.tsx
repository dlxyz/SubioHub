...
import { BarChart3, Building2, HandCoins, LayoutDashboard, LogOut, Menu, ShieldCheck, User, Wallet, X } from 'lucide-react';
import { useAuthStore } from '@/store/auth';
import { cn } from '@/lib/utils';

const AGENT_ROLES = new Set(['admin', 'agent', 'distributor']);
...
export default function AgentLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      router.replace(`/agent/login?redirect=${encodeURIComponent(pathname || '/agent')}`);
      return;
    }
  ...
  const pageTitle =
    agentNavigation
      ...
      .find((item) => pathname === item.href || (item.href !== '/agent' && pathname.startsWith(`${item.href}/`)))?.label || '代理后台';

  if (!mounted) {
    return <div className="min-h-screen bg-gray-50 dark:bg-[#121212]" />;
  }

  if (!isAuthenticated || !AGENT_ROLES.has(user?.role || '')) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 px-6 dark:bg-[#121212]">
        <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm dark:border-gray-800 dark:bg-[#1A1A1A]">
          <h1 className="text-xl font-semibold text-gray-900 dark:text-white">代理后台加载中</h1>
          <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
            {!isAuthenticated ? '正在跳转到代理登录页。' : '当前账号没有代理后台权限，正在返回个人后台。'}
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#121212]">
      ...
      >
        <div className="flex h-20 items-center justify-between border-b border-gray-200/70 px-5 dark:border-gray-800/70">
          <Link href="/agent" className="flex min-w-0 items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 text-white shadow-lg shadow-amber-500/20">
              <HandCoins className="h-5 w-5" />
            </div>
            <div className="min-w-0">
              <div className="truncate text-lg font-bold tracking-tight text-gray-900 dark:text-white">Agent Console</div>
              <div className="truncate text-xs text-gray-500 dark:text-gray-400">独立代理后台</div>
            </div>
          </Link>
          <button
        ...
        <nav className="flex-1 space-y-6 overflow-y-auto px-4 py-6">
                    ...
                    >
                      <item.icon
                        className={cn(
                          ...
                          isActive ? 'text-amber-600 dark:text-amber-300' : 'text-gray-400 dark:text-gray-500'
                        )}
                      />
                      <div className="min-w-0">
                        ...
                        <div
                          className={cn(
                            ...
                            isActive ? 'text-amber-700/80 dark:text-amber-300/80' : 'text-gray-500 dark:text-gray-400'
                          )}
                        >
                          {item.desc}
                        </div>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <div>
                <div>个人后台</div>
                <div className="mt-0.5 text-xs text-blue-500/80 dark:text-blue-300/70">切换到个人系统管理</div>
              </div>
            </Link>
        ...
        <div className="border-t border-gray-200/70 p-4 dark:border-gray-800/70">
          <div className="rounded-2xl bg-gray-50 p-3 dark:bg-[#1F1F1F]">
            <div className="text-sm font-medium text-gray-900 dark:text-white">{user?.email || 'agent'}</div>
            <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">角色: {user?.role || 'agent'}</div>
          </div>
          <button
          ...
          <div className="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">代理后台独立目录入口</div>
              </div>
            </div>
