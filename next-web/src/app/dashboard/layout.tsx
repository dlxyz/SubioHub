    ...
    items: [
      {
        ...
        descKey: "dashboard.navDesc.dashboard",
      },
      {
        key: "dashboard.nav.modelHub",
        href: "/dashboard/models",
        icon: Bot,
        descKey: "dashboard.navDesc.modelHub",
      },
      {
        key: "dashboard.nav.plans",
        href: "/dashboard/plans",
        icon: PackageCheck,
        descKey: "dashboard.navDesc.plans",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        href: "/dashboard/keys",
        icon: Key,
        descKey: "dashboard.navDesc.apiKeys",
      },
      {
        key: "dashboard.nav.usage",
        href: "/dashboard/usage",
        icon: BarChart3,
        descKey: "dashboard.navDesc.usage",
      },
      {
        key: "dashboard.nav.docsCenter",
        href: "/dashboard/docs",
        icon: BookOpen,
        descKey: "dashboard.navDesc.docsCenter",
      },
    ],
  },
  {
    ...
    items: [
      {
        key: "dashboard.nav.subscriptions",
        href: "/dashboard/subscriptions",
        icon: CreditCard,
        descKey: "dashboard.navDesc.subscriptions",
      },
      {
        key: "dashboard.nav.orders",
        href: "/dashboard/orders",
        icon: ReceiptText,
        descKey: "dashboard.navDesc.orders",
      },
      {
        key: "dashboard.nav.finance",
        href: "/dashboard/finance",
        icon: Wallet,
        descKey: "dashboard.navDesc.finance",
      },
      {
        key: "dashboard.nav.redeem",
        href: "/dashboard/redeem",
        icon: Gift,
        descKey: "dashboard.navDesc.redeem",
      },
      {
        key: "dashboard.nav.affiliate",
        href: "/dashboard/affiliate",
        icon: Users,
        descKey: "dashboard.navDesc.affiliate",
      },
      {
        key: "dashboard.nav.settings",
        href: "/dashboard/settings",
        icon: Settings,
        descKey: "dashboard.navDesc.settings",
      },
      {
        key: "dashboard.nav.aboutPlatform",
        href: "/dashboard/about",
        icon: Info,
        descKey: "dashboard.navDesc.aboutPlatform",
      },
    ],
  },
];
      ...
      <div className="flex min-h-screen items-center justify-center bg-gray-50 px-6 dark:bg-[#121212]">
        <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm dark:border-gray-800 dark:bg-[#1A1A1A]">
          <h1 className="text-xl font-semibold text-gray-900 dark:text-white">
            {t("dashboard.common.notLoggedIn")}
          </h1>
          <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
            {t("dashboard.common.redirectingToLogin")}
          </p>
        </div>
      </div>
    );
        ...
        <nav className="flex-1 space-y-6 overflow-y-auto px-4 py-6">
            ...
            >
              ...
              <div>
                ...
                <div className="mt-0.5 text-xs text-purple-500/80 dark:text-purple-300/70">
                  {t("dashboard.nav.switchAdminHint")}
                </div>
              </div>
            </Link>
          )}

          {user?.role === "agent" || user?.role === "distributor" ? (
            <Link
              href="/agent"
              onClick={() => {
                setSidebarOpen(false);
              ...
              className="flex items-center rounded-2xl border border-amber-200/70 px-3 py-3 text-sm font-medium text-amber-700 transition hover:bg-amber-50 dark:border-amber-500/20 dark:text-amber-300 dark:hover:bg-amber-500/10"
            >
              <HandCoins className="mr-3 h-5 w-5 text-amber-500" />
              <div>
                <div>{t("dashboard.nav.agent")}</div>
                <div className="mt-0.5 text-xs text-amber-500/80 dark:text-amber-300/70">
                  {t("dashboard.nav.switchAgentHint")}
                </div>
              </div>
            </Link>
          ) : null}
          ...
          {user?.role === "channel_partner" ? (
            <Link
              ...
              className="flex items-center rounded-2xl border border-teal-200/70 px-3 py-3 text-sm font-medium text-teal-700 transition hover:bg-teal-50 dark:border-teal-500/20 dark:text-teal-300 dark:hover:bg-teal-500/10"
            >
              <Building2 className="mr-3 h-5 w-5 text-teal-500" />
              <div>
                <div>{t("dashboard.nav.channel")}</div>
                <div className="mt-0.5 text-xs text-teal-500/80 dark:text-teal-300/70">
                  {t("dashboard.nav.switchChannelHint")}
                </div>
              </div>
            </Link>
          ) : null}
            ...
            <div className="flex items-center px-1 py-1">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-gray-500 dark:text-gray-400">
                  {user?.email || t("dashboard.common.notLoggedIn")}
                </p>
              </div>
            </div>
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className="hidden text-left md:block">
                    <div className="max-w-[140px] truncate text-sm font-medium text-gray-900 dark:text-white">
                      {displayName}
                    </div>
                  ...
                  <ChevronDown
                    className={cn(
                      "hidden h-4 w-4 text-gray-400 transition md:block",
