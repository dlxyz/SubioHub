...
type SortKey =
  "id" | "email" | "role" | "status" | "balance" | "concurrency" | "created_at";
...
function ModalShell({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-[2px]">
      <div
        className={`w-full ${maxWidth} overflow-hidden rounded-[28px] bg-white shadow-2xl dark:bg-[#111111]`}
      >
        <div className="flex items-center justify-between border-b border-gray-200 px-7 py-6 dark:border-gray-800">
          <h3 className="text-[32px] font-semibold tracking-[-0.03em] text-gray-900 dark:text-white">
            {title}
          </h3>
          <button
            type="button"
...
export default function AdminUsersPage() {
  ...
  const [search, setSearch] = useState(
    () =>
      readStoredObject<{ search?: string }>(FILTER_VALUES_KEY)?.search || "",
  );
  ...
  const [hiddenColumns, setHiddenColumns] = useState<Set<ColumnKey>>(
    () =>
      new Set(
        readStoredArray<ColumnKey>(HIDDEN_COLUMNS_KEY) ||
          DEFAULT_HIDDEN_COLUMNS,
      ),
  );
  const [filters, setFilters] = useState<FiltersState>(() => {
    const stored = readStoredObject<FiltersState & { search?: string }>(
      FILTER_VALUES_KEY,
    );
    return {
      role: stored?.role || "",
  ...
  const [sortState, setSortState] = useState<{
    ...
    const stored = readStoredObject<{ key?: SortKey; order?: "asc" | "desc" }>(
      SORT_STATE_KEY,
    );
    return stored?.key && stored?.order
      ? { key: stored.key, order: stored.order }
  ...
  const [historyData, setHistoryData] = useState<AdminUserBalanceHistoryResult>(
    EMPTY_BALANCE_HISTORY,
  );
  const [historyLoading, setHistoryLoading] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<AdminUser | null>(null);
  const [deleteSubmitting, setDeleteSubmitting] = useState(false);
  const [usageStats, setUsageStats] = useState<
    Record<number, AdminUserUsageStats>
  >({});
    ...
    (
      ...
      try {
        localStorage.setItem(
          VISIBLE_FILTERS_KEY,
          JSON.stringify([...nextVisibleFilters]),
        );
        localStorage.setItem(
          FILTER_VALUES_KEY,
          JSON.stringify({
        ...
        if (userResult.items.length > 0) {
          const usageEntries = await Promise.all(
            userResult.items.map(async (user) => {
              try {
                const stats = await getAdminUserUsageStats(user.id, {
                  period: "month",
                });
  ...
  const handleSwitchRole = async (
    user: AdminUser,
    role: "user" | "channel_partner" | "agent" | "distributor",
  ) => {
    try {
      ...
      if (nextRole !== role) {
        throw new Error(
          "角色更新请求已返回，但当前后端服务还没有应用新的角色更新逻辑，请重启后端服务后再试。",
        );
      }
      setUsers((prev) =>
  ...
  const handleSubmitBalance = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        getErrorMessage(
          balanceError,
          balanceOperation === "add" ? "充值失败" : "退款失败",
        ),
      );
    } finally {
  ...
  const handleSaveGroups = async () => {
    ...
    setSuccess("");
    try {
      await updateAdminUser(groupTarget.id, {
        allowed_groups: selectedGroupIds,
      });
      setSuccess("用户分组已更新");
      setGroupTarget(null);
      await loadUsers(pagination.page);
  ...
  const openApiKeysModal = async (user: AdminUser) => {
    ...
    try {
      const result = await listAdminUserApiKeys(user.id, {
        page: 1,
        page_size: 100,
      });
  ...
  const openHistoryModal = async (user: AdminUser) => {
    ...
    try {
      const result = await listAdminUserBalanceHistory(user.id, {
        page: 1,
        page_size: 20,
      });
  ...
  const toggleBuiltInFilter = (
    key: (typeof BUILT_IN_FILTERS)[number]["key"],
  ) => {
  ...
  const visibleColumnCount = useMemo(
    () =>
      TOGGLEABLE_COLUMNS.filter((column) => !hiddenColumns.has(column.key))
        .length + 2,
    [hiddenColumns],
  );
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-[#1A1A1A]">
        <div className="border-b border-gray-100 px-5 py-4 text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400">
          当前显示 {visibleColumnCount} 列，第 {pagination.page} / {totalPages}{" "}
          页，共 {pagination.total} 个用户
        </div>
            ...
            <thead className="bg-gray-50 dark:bg-gray-900/40">
              <tr className="text-left text-gray-500 dark:text-gray-400">
                ...
                {isColumnVisible("balance") && (
                  <th
                    className="cursor-pointer px-4 py-3 font-medium"
                    onClick={() => void handleSort("balance")}
                  >
                ...
                {isColumnVisible("concurrency") && (
                  <th
                    className="cursor-pointer px-4 py-3 font-medium"
                    onClick={() => void handleSort("concurrency")}
                  >
                ...
                {isColumnVisible("created_at") && (
                  <th
                    className="cursor-pointer px-4 py-3 font-medium"
                    onClick={() => void handleSort("created_at")}
                  >
                    ...
                    {isColumnVisible("usage") && (
                      <td className="px-4 py-4">
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          <div>
                            本月{" "}
                            {formatCurrency(usageStats[user.id]?.total_cost)}
                          </div>
                          <div className="mt-1">
                            请求{" "}
                            {Number(usageStats[user.id]?.total_requests || 0)}
                          </div>
                        </div>
                      </td>
                    )}
                          ...
                          <>
                            {user.role !== "user" && (
                              <button
                                type="button"
                                onClick={() =>
                                  void handleSwitchRole(user, "user")
                                }
                            ...
                            {user.role !== "agent" && (
                              <button
                                type="button"
                                onClick={() =>
                                  void handleSwitchRole(user, "agent")
                                }
                            ...
                            {user.role !== "channel_partner" && (
                              <button
                                type="button"
                                onClick={() =>
                                  void handleSwitchRole(user, "channel_partner")
                                }
                            ...
                            {user.role !== "distributor" && (
                              <button
                                type="button"
                                onClick={() =>
                                  void handleSwitchRole(user, "distributor")
                                }
        ...
        <form onSubmit={handleSubmitBalance}>
          <div className="space-y-5 px-7 py-6">
            <div className="text-sm text-gray-500 dark:text-gray-400">
              目标用户：{balanceTarget?.email}
            </div>
            <div>
              <FieldLabel>
                {balanceOperation === "add" ? "充值金额" : "退款金额"}
              </FieldLabel>
              <input
                required
                type="number"
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setBalanceForm((prev) => ({ ...prev, notes: e.target.value }))
                }
                placeholder="请输入操作备注"
                className="w-full rounded-2xl border border-gray-200 bg-transparent px-5 py-4 text-base text-gray-900 outline-none transition focus:border-emerald-400 dark:border-gray-700 dark:text-white"
              />
      ...
      >
        <div className="space-y-5 px-7 py-6">
          <div className="text-sm text-gray-500 dark:text-gray-400">
            为用户 `{groupTarget?.email}` 选择允许访问的分组。
          </div>
                ...
                <tbody>
                  {apiKeys.map((apiKey) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400">
                        {formatCurrency(apiKey.quota_used)} /{" "}
                        {apiKey.quota ? formatCurrency(apiKey.quota) : "不限"}
                      </td>
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400">
