...
import { GroupMessagesDispatchFields } from '@/components/admin/group-messages-dispatch-fields';
import {
...
import {
  ...
  createMessagesDispatchMappingRow,
  getMessagesDispatchPreset,
  getRecommendedMessagesDispatchPresetKind,
  isMessagesDispatchFormStateEquivalent,
  messagesDispatchConfigToFormState,
  messagesDispatchFormStateToConfig,
  type MessagesDispatchPresetKind,
  type MessagesDispatchFormState,
} from '@/lib/group-messages-dispatch';
...
type GroupFormState = {
  ...
  messages_dispatch: MessagesDispatchFormState;
...
const DEFAULT_GROUP_FORM: GroupFormState = {
  ...
  description: '',
  platform: 'anthropic',
  routing_profile: 'overseas',
  allow_messages_dispatch: false,
  default_mapped_model: getMessagesDispatchPreset('overseas').defaultMappedModel,
  messages_dispatch: createDefaultMessagesDispatchFormState('overseas'),
  rate_multiplier: '1',
  is_exclusive: false,
...
function getSchedulerPlatformHelp(platform?: string) {
  switch (platform) {
    case 'anthropic':
      return '用于 Claude / Anthropic 账号调度。国内渠道可额外单独绑定该分组。';
    case 'openai':
      return '用于 OpenAI 账号调度。国内渠道可额外单独绑定该分组。';
    case 'gemini':
      return '用于 Gemini 账号调度。国内渠道可额外单独绑定该分组。';
    case 'antigravity':
      return '用于 Antigravity 混合调度。国内渠道可额外单独绑定该分组。';
    default:
      return '该字段定义原系统的主调度平台，不限制国内渠道绑定。';
  }
}
...
function getRoutingProfileLabel(profile?: string) {
  switch (profile) {
    case 'domestic':
      return '国内渠道组';
    case 'mixed':
...
function getRoutingProfileBadgeClass(profile?: string) {
  switch (profile) {
    case 'domestic':
...
function getRoutingProfileHelp(profile?: string) {
  switch (profile) {
    case 'domestic':
      return '主要给国内渠道使用，建议用于 DeepSeek / Qwen / 豆包 / 智谱 等真实上游调试。';
    case 'mixed':
      return '同时允许海外账号调度和国内渠道接入，适合灰度迁移或混合能力分组。';
    default:
      return '主要用于原有海外/订阅账号调度链路；如果还要挂国内渠道，建议改成混合组。';
  }
}

function getMessagesDispatchBadgeClass(enabled?: boolean) {
  return enabled
    ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300'
    : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300';
}

function getMessagesDispatchSummary(group: AdminGroup) {
  if (group.platform !== 'openai') {
    return {
      label: '不适用',
      detail: `${getPlatformLabel(group.platform)} 组不可配置`,
      className: 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300',
    };
  }
  return {
    label: group.allow_messages_dispatch ? '已开启' : '未开启',
    detail: group.allow_messages_dispatch ? '允许 /v1/messages' : '拒绝 /v1/messages',
    className: getMessagesDispatchBadgeClass(group.allow_messages_dispatch),
  };
}

function normalizeModelOptions(values: Array<string | undefined | null>) {
  return Array.from(
    new Set(
      values
...
function buildMessagesDispatchModelOptions(channels: AdminChannel[], presetKind: MessagesDispatchPresetKind) {
  const preset = getMessagesDispatchPreset(presetKind);
  const commonDomesticModels = [
    'deepseek-v4-pro',
    'deepseek-v4-flash',
    'deepseek-chat',
    'qwen-plus',
  ...
  const commonOverseasModels = ['gpt-5.4', 'gpt-5.3-codex', 'gpt-5.4-mini', 'gpt-4.1', 'gpt-4o-mini'];
  const commonModels = presetKind === 'domestic' ? commonDomesticModels : commonOverseasModels;
  const channelModels = channels.flatMap((channel) => channel.model_pricing?.flatMap((pricing) => pricing.models || []) || []);

  return {
    defaultMappedModel: normalizeModelOptions([preset.defaultMappedModel, ...commonModels, ...channelModels]),
    opus: normalizeModelOptions([preset.config.opus_mapped_model, ...commonModels, ...channelModels]),
    sonnet: normalizeModelOptions([preset.config.sonnet_mapped_model, ...commonModels, ...channelModels]),
    haiku: normalizeModelOptions([preset.config.haiku_mapped_model, ...commonModels, ...channelModels]),
    exactTarget: normalizeModelOptions([
      preset.defaultMappedModel,
      preset.config.opus_mapped_model,
      preset.config.sonnet_mapped_model,
      preset.config.haiku_mapped_model,
      ...commonModels,
      ...channelModels,
    ]),
  };
...
function buildDomesticChannelBindings(channels: AdminChannel[]) {
  const result: Record<number, string[]> = {};
  for (const channel of channels || []) {
    for (const groupID of channel.group_ids || []) {
      if (!result[groupID]) result[groupID] = [];
      result[groupID].push(channel.name || `渠道#${channel.id}`);
    }
  }
  for (const key of Object.keys(result)) {
    result[Number(key)] = Array.from(new Set(result[Number(key)]));
  }
  return result;
}

function toGroupForm(group: AdminGroup): GroupFormState {
  const routingProfile = group.routing_profile || 'overseas';
  const presetKind = getRecommendedMessagesDispatchPresetKind(routingProfile);
  const preset = getMessagesDispatchPreset(presetKind);
  return {
    name: group.name || '',
    ...
    allow_messages_dispatch: Boolean(group.allow_messages_dispatch),
    default_mapped_model: group.default_mapped_model || preset.defaultMappedModel,
    messages_dispatch: messagesDispatchConfigToFormState(group.messages_dispatch_model_config, presetKind),
...
function applyMessagesDispatchPreset(form: GroupFormState, presetKind: MessagesDispatchPresetKind): GroupFormState {
  const preset = getMessagesDispatchPreset(presetKind);
  return {
    ...form,
    default_mapped_model: preset.defaultMappedModel,
    messages_dispatch: preset.config,
  };
}

function isUsingRecommendedMessagesDispatchPreset(form: GroupFormState, presetKind: MessagesDispatchPresetKind) {
  const preset = getMessagesDispatchPreset(presetKind);
  return (
    form.default_mapped_model.trim() === preset.defaultMappedModel &&
    isMessagesDispatchFormStateEquivalent(form.messages_dispatch, preset.config)
  );
}
...
export default function AdminGroupsPage() {
  ...
  const [domesticChannelBindings, setDomesticChannelBindings] = useState<Record<number, string[]>>({});
  const [domesticChannels, setDomesticChannels] = useState<AdminChannel[]>([]);
  ...
  const loadGroups = useCallback(
    async (page = 1, nextSearch = search, nextFilters = filters, nextSort = sortState) => {
      ...
      setError('');
      try {
        const [groupResult, usageResult, capacityResult, allGroupResult, domesticChannelResult] = await Promise.all([
          listAdminGroups({
            page,
            page_size: PAGE_SIZE,
          ...
          listAdminChannels({ page: 1, page_size: 500, provider_scope: 'domestic', sort_by: 'created_at', sort_order: 'desc' }),
        ]);
        ...
        setDomesticChannelBindings(buildDomesticChannelBindings(domesticChannelResult.items));
        setDomesticChannels(domesticChannelResult.items);
  ...
  const messagesDispatchModelOptions = useMemo(
    () => buildMessagesDispatchModelOptions(domesticChannels, getRecommendedMessagesDispatchPresetKind(groupForm.routing_profile)),
    [domesticChannels, groupForm.routing_profile]
  );
  ...
  const handleSubmitGroup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload = {
        ...
        routing_profile: groupForm.routing_profile,
        allow_messages_dispatch: groupForm.allow_messages_dispatch,
        default_mapped_model: groupForm.default_mapped_model.trim() || undefined,
        messages_dispatch_model_config: messagesDispatchFormStateToConfig(groupForm.messages_dispatch),
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-900/40">
              <tr className="text-left text-gray-500 dark:text-gray-400">
                ...
                <th className="px-4 py-3 font-medium">主调度平台</th>
                <th className="px-4 py-3 font-medium">计费类型</th>
                <th className="px-4 py-3 font-medium">费率倍数</th>
                <th className="px-4 py-3 font-medium">国内渠道绑定</th>
            ...
            <tbody>
              {loading ? (
                ...
                groups.map((group) => {
                  ...
                  const domesticBindings = domesticChannelBindings[group.id] || [];
                  const messagesDispatchSummary = getMessagesDispatchSummary(group);
                  ...
                  return (
                    <tr key={group.id} className="border-t border-gray-100 align-top dark:border-gray-800">
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${getRoutingProfileBadgeClass(
                            group.routing_profile
                          )}`}
                        >
                          {getRoutingProfileLabel(group.routing_profile)}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <div className="space-y-1 text-xs">
                          <span className={`inline-flex items-center rounded-full px-2.5 py-1 font-medium ${messagesDispatchSummary.className}`}>
                            {messagesDispatchSummary.label}
                          </span>
                          <div className="text-gray-500 dark:text-gray-400">{messagesDispatchSummary.detail}</div>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <span
                      ...
                      <td className="px-4 py-4">
                        {domesticBindings.length > 0 ? (
                          <div className="space-y-1 text-xs">
                            <div className="font-medium text-emerald-700 dark:text-emerald-300">{domesticBindings.length} 条国内渠道</div>
                            <div className="text-gray-500 dark:text-gray-400">{domesticBindings.slice(0, 3).join(' / ')}</div>
                            {domesticBindings.length > 3 ? (
                              <div className="text-gray-400 dark:text-gray-500">另有 {domesticBindings.length - 3} 条未展开</div>
                            ) : null}
                          </div>
                        ) : (
                          <div className="text-xs text-gray-400 dark:text-gray-500">未绑定</div>
                        )}
                      </td>
                      <td className="px-4 py-4">
                        <span
        ...
        <form onSubmit={handleSubmitGroup}>
          <div className="space-y-6 px-7 py-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <FieldLabel>主调度平台</FieldLabel>
                <select
                  value={groupForm.platform}
                  onChange={(e) =>
                    setGroupForm((prev) => {
                      const nextPlatform = e.target.value;
                      if (nextPlatform !== 'openai' || prev.platform === 'openai') {
                        return { ...prev, platform: nextPlatform };
                      }
                      const presetKind = getRecommendedMessagesDispatchPresetKind(prev.routing_profile);
                      return applyMessagesDispatchPreset({ ...prev, platform: nextPlatform }, presetKind);
                    })
                  }
                  className="w-full rounded-2xl border border-gray-200 bg-transparent px-5 py-4 text-base text-gray-900 outline-none transition focus:border-emerald-400 dark:border-gray-700 dark:text-white"
                >
                ...
                <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">{getSchedulerPlatformHelp(groupForm.platform)}</p>
              </div>
              <div>
                <FieldLabel>用途 / 路由类型</FieldLabel>
                <select
                  value={groupForm.routing_profile}
                  onChange={(e) =>
                    setGroupForm((prev) => {
                      const nextRoutingProfile = e.target.value;
                      const nextForm = { ...prev, routing_profile: nextRoutingProfile };
                      if (prev.platform !== 'openai') {
                        return nextForm;
                      }
                      const previousPresetKind = getRecommendedMessagesDispatchPresetKind(prev.routing_profile);
                      if (!isUsingRecommendedMessagesDispatchPreset(prev, previousPresetKind)) {
                        return nextForm;
                      }
                      return applyMessagesDispatchPreset(nextForm, getRecommendedMessagesDispatchPresetKind(nextRoutingProfile));
                    })
                  }
                  className="w-full rounded-2xl border border-gray-200 bg-transparent px-5 py-4 text-base text-gray-900 outline-none transition focus:border-emerald-400 dark:border-gray-700 dark:text-white"
                >
                  <option value="overseas">海外主调度组</option>
                  <option value="domestic">国内渠道组</option>
                  <option value="mixed">混合组</option>
                </select>
                <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">{getRoutingProfileHelp(groupForm.routing_profile)}</p>
              </div>
            </div>
            ...
            <GroupMessagesDispatchFields
              supported={groupForm.platform === 'openai'}
              platformLabel={getPlatformLabel(groupForm.platform)}
              routingProfile={groupForm.routing_profile}
              enabled={groupForm.allow_messages_dispatch}
              defaultMappedModel={groupForm.default_mapped_model}
              config={groupForm.messages_dispatch}
              recommendedPresetKind={getRecommendedMessagesDispatchPresetKind(groupForm.routing_profile)}
              modelOptions={messagesDispatchModelOptions}
              onToggleEnabled={() =>
                setGroupForm((prev) => ({ ...prev, allow_messages_dispatch: !prev.allow_messages_dispatch }))
              }
              onApplyPreset={(presetKind) =>
                setGroupForm((prev) => applyMessagesDispatchPreset(prev, presetKind))
              }
              onChangeDefaultMappedModel={(value) =>
                setGroupForm((prev) => ({ ...prev, default_mapped_model: value }))
              }
              onChangeFamilyModel={(key, value) =>
                setGroupForm((prev) => ({
                  ...prev,
                  messages_dispatch: {
                    ...prev.messages_dispatch,
                    [key]: value,
                  },
                }))
              }
              onAddExactMapping={() =>
                setGroupForm((prev) => ({
                  ...
                  messages_dispatch: {
                    ...prev.messages_dispatch,
                    exact_model_mappings: [...prev.messages_dispatch.exact_model_mappings, createMessagesDispatchMappingRow()],
                  },
                }))
              }
              onUpdateExactMapping={(id, key, value) =>
                setGroupForm((prev) => ({
                  ...prev,
                  messages_dispatch: {
                    ...prev.messages_dispatch,
                    exact_model_mappings: prev.messages_dispatch.exact_model_mappings.map((row) =>
                      row.id === id ? { ...row, [key]: value } : row
                    ),
                  },
                }))
              }
              onRemoveExactMapping={(id) =>
                setGroupForm((prev) => ({
                  ...prev,
                  messages_dispatch: {
                    ...prev.messages_dispatch,
                    exact_model_mappings: prev.messages_dispatch.exact_model_mappings.filter((row) => row.id !== id),
                  },
                }))
              }
            />

            <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700 dark:border-blue-900/50 dark:bg-blue-950/30 dark:text-blue-300">
              `主调度平台` 只描述原系统账号池主要走哪类海外调度；`用途 / 路由类型` 才是你现在区分国内组、海外组、混合组的字段。
              创建 `DeepSeek / Qwen / 豆包 / 智谱` 等国内渠道时，建议把分组用途设为 `国内渠道组` 或 `混合组`，再去国内渠道页绑定这个分组。
            </div>
