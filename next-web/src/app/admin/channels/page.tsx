...
export default function AdminChannelsPage() {
  ...
  const isEligibleOverseasGroup = useCallback((group: AdminGroup) => group.routing_profile !== 'domestic', []);

  const groupsByPlatform = useMemo(() => {
    const entries = PLATFORM_OPTIONS.map((platform) => [
      platform.key,
      groups.filter((group) => group.platform === platform.key && isEligibleOverseasGroup(group)),
    ]);
    return Object.fromEntries(entries) as Record<string, AdminGroup[]>;
  }, [groups, isEligibleOverseasGroup]);
      ...
      try {
        const [channelResult, groupResult] = await Promise.all([
          listAdminChannels({
            ...
            provider_scope: 'overseas',
            sort_by: 'created_at',
            sort_order: 'desc',
          }),
          groups.length > 0 ? Promise.resolve({ items: groups, total: groups.length, page: 1, pageSize: groups.length }) : listAdminGroups({ page: 1, page_size: 500 }),
        ]);
        ...
        setError(getErrorMessage(loadError, '加载订阅配置失败'));
      } finally {
        setLoading(false);
      }
    },
  ...
  const openEditModal = useCallback(async (channel: AdminChannel) => {
    setError('');
    try {
      ...
      for (const groupId of detail.group_ids || []) {
        ...
        if (targetGroup?.platform && isEligibleOverseasGroup(targetGroup)) {
      ...
      setError(getErrorMessage(editError, '加载订阅详情失败'));
    }
  }, [groups, isEligibleOverseasGroup]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(getErrorMessage(submitError, editingChannel ? '更新订阅失败' : '创建订阅失败'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleToggleStatus = async (channel: AdminChannel) => {
    try {
      ...
      setSuccess(nextStatus === 'active' ? '订阅已启用' : '订阅已停用');
      await loadChannels(pagination.page);
    } catch (statusError: unknown) {
      setError(getErrorMessage(statusError, '更新订阅状态失败'));
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(deleteError, '删除订阅失败'));
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">维护海外订阅类上游与原有账号调度主链，模型会同步汇总到模型速览。</p>
        </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-[#1A1A1A]">
        <div className="border-b border-gray-100 px-5 py-4 text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400">
          第 {pagination.page} / {totalPages} 页，共 {pagination.total} 个订阅渠道
        </div>
      ...
      <ModalShell open={showModal} title={editingChannel ? '编辑订阅' : '创建订阅'} onClose={closeModal}>
          ...
          <div className="space-y-6 px-7 py-6">
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-2xl border border-gray-200 bg-transparent px-5 py-4 text-base text-gray-900 outline-none transition focus:border-emerald-400 dark:border-gray-700 dark:text-white"
              >
                <option value="channel_mapped">以订阅映射后的模型计费</option>
            ...
            <div className="border-t border-gray-200 pt-4 dark:border-gray-800">
              ...
              <div className="mb-3 text-xs text-gray-500 dark:text-gray-400">
                订阅管理这里只显示 `海外主调度组` 和 `混合组`。标记为 `国内渠道组` 的分组不会出现在这里。
              </div>
              <div className="flex flex-wrap gap-2">
                          ...
                          {(form.modelMappings[platform] || []).map((row, index) => (
                            <div key={`${platform}-${index}`} className="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                              ...
                              <input
                                ...
                                placeholder="映射后的模型名，例如 claude-sonnet-4-20250514"
                                className="rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-emerald-400 dark:border-gray-700 dark:bg-[#111111] dark:text-white"
                              />
                              <button
                                type="button"
            ...
            <div className="rounded-2xl border border-gray-200 p-5 dark:border-gray-800">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">开启后，订阅定价也会参与账号统计口径。</div>
                </div>
                <button
                  type="button"
                  onClick={() =>
      ...
      <ModalShell open={Boolean(deleteTarget)} title="删除订阅渠道" onClose={() => setDeleteTarget(null)}>
        <div className="space-y-5 px-7 py-6">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            确认删除订阅渠道 <span className="font-medium text-gray-900 dark:text-white">{deleteTarget?.name}</span> 吗？该操作不可撤销。
          </p>
        </div>
        <div className="flex justify-end gap-3 border-t border-gray-200 px-7 py-5 dark:border-gray-800">
          <button
