...
import {
  ...
  listAdminAffiliateCommissionSplits,
  listAdminUsers,
  settleAdminAffiliateCommission,
  settleAdminAffiliateCommissionSplit,
  updateAdminAffiliateUserRate,
  type AdminCommissionLog,
  type AdminCommissionSplitLog,
  type SystemSettings,
  type AdminUser,
} from '@/lib/admin-api';

type WithdrawalManagementPanelProps = {
  title: string;
  description: string;
  showRateSection?: boolean;
};
...
export function WithdrawalManagementPanel({
  title,
  description,
  showRateSection = false,
}: WithdrawalManagementPanelProps) {
  const [commissions, setCommissions] = useState<AdminCommissionLog[]>([]);
  const [splitLogs, setSplitLogs] = useState<AdminCommissionSplitLog[]>([]);
  ...
  const [splitSettlingId, setSplitSettlingId] = useState<number | null>(null);
  ...
  const [splitFilters, setSplitFilters] = useState({
    status: '',
    beneficiaryUserId: '',
    beneficiaryRole: '',
  });
  ...
  const [splitPagination, setSplitPagination] = useState({
    total: 0,
    page: 1,
    pageSize: PAGE_SIZE,
  });
  ...
  const loadCommissions = useCallback(async (nextPage = 1, nextFilters = filters) => {
    ...
    try {
      ...
      setError(getErrorMessage(loadError, '加载佣金流水失败'));
    } finally {
      setLoading(false);
    }
  }, [filters]);

  const loadSplitLogs = useCallback(async (nextPage = 1, nextFilters = splitFilters) => {
    setSplitLoading(true);
    setError('');
    try {
      const result = await listAdminAffiliateCommissionSplits({
        page: nextPage,
        page_size: PAGE_SIZE,
        status: nextFilters.status || undefined,
        beneficiary_user_id: nextFilters.beneficiaryUserId || undefined,
        beneficiary_role: nextFilters.beneficiaryRole || undefined,
      });
      setSplitLogs(result.items);
      setSplitPagination({
        total: result.total,
        page: result.page || nextPage,
        pageSize: result.pageSize || PAGE_SIZE,
      ...
      setError(getErrorMessage(loadError, '加载补差法分润明细失败'));
    } finally {
  ...
  const loadUsers = useCallback(async (keyword = '') => {
    ...
    try {
      ...
      setError(getErrorMessage(loadError, '加载返佣用户列表失败'));
    } finally {
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      setError(getErrorMessage(loadError, '加载分销结算设置失败'));
    }
  }, []);
  ...
  }, [loadCommissions, loadSplitLogs, loadUsers, loadSettings, showRateSection]);
  ...
  const splitSummary = useMemo(() => {
    const pendingCount = splitLogs.filter((item) => item.status === 'pending').length;
    const pendingAmount = splitLogs
      .filter((item) => item.status === 'pending')
      .reduce((sum, item) => sum + Number(item.commission_amount || 0), 0);
    const settledAmount = splitLogs
      .filter((item) => item.status === 'settled')
      .reduce((sum, item) => sum + Number(item.commission_amount || 0), 0);
    return {
      pendingCount,
  ...
  const splitTotalPages = Math.max(1, Math.ceil((splitPagination.total || 0) / (splitPagination.pageSize || PAGE_SIZE)));
  ...
  const handleSplitFilterSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setSuccess('');
    await loadSplitLogs(1, splitFilters);
  };

  const handleResetSplitFilters = async () => {
    const nextFilters = { status: '', beneficiaryUserId: '', beneficiaryRole: '' };
    setSplitFilters(nextFilters);
    setSuccess('');
    await loadSplitLogs(1, nextFilters);
  };
  ...
  const handleSettle = async (commission: AdminCommissionLog) => {
    ...
    try {
      ...
      setError(getErrorMessage(settleError, '结算佣金失败'));
    } finally {
      setSettlingId(null);
    }
  };

  const handleSplitSettle = async (splitLog: AdminCommissionSplitLog) => {
    if (settings?.affiliate_manual_payout_settlement_enabled === false) {
      setError('系统已关闭“手动打款后结算”，请先前往系统设置开启后再操作');
      return;
    }
    setSplitSettlingId(splitLog.id);
    setError('');
    setSuccess('');
    try {
      await settleAdminAffiliateCommissionSplit(splitLog.id);
      setSuccess(`补差法分润明细 #${splitLog.id} 已手动结算`);
      await loadSplitLogs(splitPagination.page, splitFilters);
      if (showRateSection) {
        await loadUsers(userSearch);
      }
    } catch (settleError: unknown) {
      setError(getErrorMessage(settleError, '结算补差法分润明细失败'));
    } finally {
      setSplitSettlingId(null);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {settings?.affiliate_manual_payout_settlement_enabled === false ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-300">
          当前已关闭“手动打款后结算”，提现管理页仅可查看，不能执行手动结算。请前往系统设置中的“分销结算”开启该功能。
        </div>
      ) : null}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-[#1A1A1A]">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">当前筛选条数</p>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-[#1A1A1A]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">补差法分润提现处理</h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              这里展示新底座 `commission_split_logs` 的实际分润记录，并支持对 `pending` 状态执行手动结算。
            </p>
          </div>

          <form onSubmit={handleSplitFilterSubmit} className="grid gap-3 sm:grid-cols-2 lg:flex lg:flex-wrap">
            <input
              value={splitFilters.beneficiaryUserId}
              onChange={(e) => setSplitFilters((prev) => ({ ...prev, beneficiaryUserId: e.target.value }))}
            ...
            />
            <select
              value={splitFilters.beneficiaryRole}
              onChange={(e) => setSplitFilters((prev) => ({ ...prev, beneficiaryRole: e.target.value }))}
            ...
            <select
              value={splitFilters.status}
              onChange={(e) => setSplitFilters((prev) => ({ ...prev, status: e.target.value }))}
              className="rounded-lg border border-gray-200 bg-transparent px-3 py-2 text-sm dark:border-gray-700 dark:bg-[#1A1A1A] dark:text-white"
            >
            ...
            <button
              type="button"
              onClick={handleResetSplitFilters}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-900/40">
            <div className="text-xs text-gray-500 dark:text-gray-400">当前页待结算笔数</div>
            <div className="mt-2 text-xl font-semibold text-gray-900 dark:text-white">{splitLoading ? '...' : splitSummary.pendingCount}</div>
          </div>
          <div className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-900/40">
            <div className="text-xs text-gray-500 dark:text-gray-400">当前页待结算金额</div>
            <div className="mt-2 text-xl font-semibold text-gray-900 dark:text-white">{splitLoading ? '...' : formatCurrency(splitSummary.pendingAmount)}</div>
          </div>
          <div className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-900/40">
            <div className="text-xs text-gray-500 dark:text-gray-400">当前页已结算金额</div>
            <div className="mt-2 text-xl font-semibold text-gray-900 dark:text-white">{splitLoading ? '...' : formatCurrency(splitSummary.settledAmount)}</div>
          </div>
        </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-900/40">
                <tr className="text-left text-gray-500 dark:text-gray-400">
                  <th className="px-4 py-3 font-medium">明细 ID</th>
                  <th className="px-4 py-3 font-medium">受益人</th>
                  <th className="px-4 py-3 font-medium">消费用户</th>
                  <th className="px-4 py-3 font-medium">订单</th>
                  <th className="px-4 py-3 font-medium">层级/角色</th>
                  <th className="px-4 py-3 font-medium">比例信息</th>
                  <th className="px-4 py-3 font-medium">分润金额</th>
                    ...
                    <tr key={item.id} className="border-t border-gray-100 dark:border-gray-800">
                      ...
                      <td className="px-4 py-3">
                        <div className="font-medium text-gray-900 dark:text-white">
                          {item.beneficiary_email || `用户 ${item.beneficiary_user_id}`}
                        </div>
                        <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">ID: {item.beneficiary_user_id}</div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-gray-900 dark:text-white">{item.consumer_user_email || `用户 ${item.consumer_user_id}`}</div>
                        <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">ID: {item.consumer_user_id}</div>
                      </td>
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400">
                        <div>#{item.order_id ?? '-'}</div>
                        <div className="mt-1 text-xs">{item.order_type || '-'} / {item.order_status || '-'}</div>
                      </td>
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400">
                        <div className="font-medium text-gray-900 dark:text-white">L{item.level}</div>
                        <div className="mt-1 text-xs">{item.beneficiary_role || '-'}</div>
                      </td>
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400">
                        <div>目标比例：{formatPercent(item.target_rate)}</div>
                        <div className="mt-1 text-xs">补差基准：{formatPercent(item.parent_rate)}</div>
                      </td>
                      <td className="px-4 py-3 text-gray-900 dark:text-white">
                        <div>{formatCurrency(item.commission_amount)}</div>
                        <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">基数 {formatCurrency(item.base_amount)}</div>
                      </td>
                      <td className="px-4 py-3">
                        ...
                        {item.settled_at ? (
                          <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">{formatDate(item.settled_at)}</div>
                        ) : null}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {item.status === 'pending' ? (
                          <button
                            onClick={() => handleSplitSettle(item)}
                            disabled={
                              splitSettlingId === item.id || settings?.affiliate_manual_payout_settlement_enabled === false
                            }
                            className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-emerald-700 disabled:opacity-60"
                          >
                            {splitSettlingId === item.id ? '结算中...' : '手动结算'}
                          </button>
                        ) : (
        ...
        <div className="mt-4 flex flex-col gap-3 border-t border-gray-100 pt-4 text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400 sm:flex-row sm:items-center sm:justify-between">
          <div>
            第 {splitPagination.page} / {splitTotalPages} 页，共 {splitPagination.total} 条
          </div>
          <div className="flex gap-2">
            <button
              type="button"
              disabled={splitPagination.page <= 1 || splitLoading}
              onClick={() => loadSplitLogs(splitPagination.page - 1, splitFilters)}
            ...
            <button
              type="button"
              disabled={splitPagination.page >= splitTotalPages || splitLoading}
              onClick={() => loadSplitLogs(splitPagination.page + 1, splitFilters)}
              className="rounded-lg border border-gray-200 px-4 py-2 text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
            >
              下一页
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-[#1A1A1A]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">推广佣金提现处理</h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              支持按受益人 ID 和状态筛选，并对旧 `commission_logs` 的 `pending` 流水执行人工结算。
            </p>
          </div>
