...
**API キー、アカウント接続、ルーティング、運用を統合する AI ゲートウェイ / モデル中継プラットフォーム**
...
SubioHub は、統一された上流アクセス、API キーや OAuth アカウントの接続、モデル中継、使用量計測、運用管理のために設計されたセルフホスト型の AI ゲートウェイプラットフォームです。OpenAI 互換の一貫したインターフェースを提供しつつ、プロバイダ接続、キー中継、アカウントプール、ルーティング、課金ルール、管理ワークフローを一つのシステムで扱えます。

## 機能

- **複数上流の接続** - OAuth や API キーを含む複数の上流認証方式をサポート
- **OpenAI 互換のモデル中継** - モデル転送、キーパススルー、上流抽象化を統一インターフェースで提供
- **APIキー配布** - ユーザー向け API キー、クォータ、アクセス範囲を管理
- **モデルルーティングとスケジューリング** - モデル、チャネル、ポリシー、スティッキーセッションに基づいて経路制御
- **精密な課金** - トークンレベルの使用量追跡、価格ルール、コスト計算
...
- **管理ダッシュボード** - チャネル、アカウント、モデル、使用量、決済、運用を一元管理
- **外部システム連携** - 外部システム（チケット管理など）を iframe 経由で管理ダッシュボードに埋め込み可能

## ユースケース

- **チーム向けゲートウェイ** - 複数の AI プロバイダを 1 つの内部 API エンドポイントの背後に統合
- **自前キー / アカウントでのモデル中継** - 自分の上流キーやアカウントを使って Claude、Gemini、OpenAI 互換などのモデルを 1 つの基盤で中継
- **サブスクリプション再販** - 下流ユーザーへクォータ、キー、ルーティングポリシーを割り当て、課金ルールで利用を制御
