...
| 数据库凭据 | user=`subiohub`, password=`subiohub`, dbname=`subiohub` |
...
psql -U subiohub -h 127.0.0.1 -d subiohub -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER subiohub WITH PASSWORD 'subiohub';
...
```bash
# 连接数据库
psql -U subiohub -h 127.0.0.1 -d subiohub
...
# 执行 SQL 文件
psql -U subiohub -h 127.0.0.1 -d subiohub -f migration.sql
```
