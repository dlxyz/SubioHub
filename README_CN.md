...
**统一的 AI 网关与模型中转平台，覆盖 API Key、账号接入、路由与运维管理**
...
SubioHub 是一个自托管的 AI 网关与模型中转平台，面向统一上游接入、API Key 与 OAuth 账号接入、模型转发、用量计量和运维管理而设计。它帮助团队以一致的 OpenAI 兼容接口对外提供服务，同时在同一套系统中完成上游对接、密钥中转、账号池调度、路由策略、计费规则和管理后台工作流。

## 功能特性

- **多上游接入** - 支持多种上游账号与凭证类型，包括 OAuth、API Key
- **OpenAI 兼容模型中转** - 通过统一接口完成模型转发、密钥透传与上游抽象
- **API Key 分发** - 为用户生成和管理 API Key、额度与访问范围
- **模型路由与调度** - 按模型、渠道、策略和粘性会话进行请求调度
- **精确计费** - Token 级别的用量追踪、定价规则与成本计算
- **并发控制** - 用户级和账号级并发限制
- **速率限制** - 可配置的请求和 Token 速率限制
- **内置支付系统** - 支持 EasyPay 易支付、支付宝官方、微信官方、Stripe，用户自助充值，无需独立部署支付服务（[配置指南](docs/PAYMENT_CN.md)）
- **管理后台** - 在 Web 界面统一管理渠道、账号、模型、用量、支付和运维
- **外部系统集成** - 支持通过 iframe 嵌入外部系统（如工单等），扩展管理后台功能

## 使用场景

- **团队统一网关** - 在一个内部 API 入口后统一接入多个 AI 提供商
- **自有 Key/账号模型中转** - 使用你自己的上游密钥或账号，将 Claude、Gemini、OpenAI 兼容等模型流量统一接入一个平台
- **订阅转售与分发** - 为下游用户分配额度、密钥和路由策略，并配合计费规则控制使用
- **运维管理后台** - 在同一面板内管理账号、渠道、代理、限流和监控
- **多语言内容发布** - 在公开站点支持多语言公告与资讯展示
...
# 查看日志（复制到记事本可以找到登录密码，用户名：admin@subiohub.com）
