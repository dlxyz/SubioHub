...
SubioHub 是一个自托管的 AI API 网关平台，面向统一上游接入、账号调度、API Key 分发、用量计量和运维管理而设计。它帮助团队以一致的 OpenAI 兼容接口对外提供服务，同时在同一套系统中完成上游接入、路由策略、计费规则和管理后台工作流。
...
- **订阅转售与分发** - 为下游用户分配额度和密钥，并配合计费规则控制使用
